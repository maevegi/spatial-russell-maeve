...
We want to compare the NDVI values in San Francisco with other cities within the Bay Area. Our city of choice is San Jose. First we need to define a new bounding box with the same time span for San Jose. 
```{r}
box2<- st_bbox(sf3)
start_date <- "2022-06-01"
end_date <- "2022-08-31"
...
We are making a raster cube and applying the calculations for NDVI.
```{r}
sf3_ndvi <- 
...
Now we are combining the NDVI polygons with the HOLC grade polygons and mapping them. 
```{r}
tmap_mode("plot")
...
Finally we are summarizing the mean NDVI values for each HOLC grade assignment. 
```{r}
