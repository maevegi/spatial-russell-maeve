...
We are utilizing the following spatial data:  https://dsl.richmond.edu/panorama/redlining/#loc=3/41.245/-105.469&text=intro

**1.Mapping Inequality:** 

```{r}
oakland_zip <- "https://dsl.richmond.edu/panorama/redlining/static/downloads/shapefiles/CAOakland1937.zip"

oakland_url <- paste0("/vsizip/vsicurl/", oakland_zip)

oak <- read_sf(oakland_url)
oak
```


```{r}
tmap_mode("view") #hashtag when knitting
```

```{r}
tm_shape(oak) + tm_polygons("holc_grade", alpha = 0.5)
```
...
#Map redlining map with NDVI image
...
```{r}
oak_avg_ndvi<-oak_ndvi|>
  extract_geom(oak, FUN=mean)
oak_avg_ndvi|>as_tibble()
oak_new<- oak|> rowid_to_column("FID")
oak_polygons<- left_join(oak_new, oak_avg_ndvi)
oak_polygons

#why are NDVI values so small? 
#fix reproducibility issues 
```

```{r}
tmap_mode("plot")
oakland_NDVI<-tm_basemap()+
tm_shape(oak_polygons)+tm_polygons("NDVI", style="quantile", palette="Greens")+
  tm_shape(oak_polygons)+ tm_text("holc_grade", size=0.5)+tm_layout(legend.width=0.5)

oakland_NDVI[holc_id=NA]<-NA
oakland_NDVI
```

```{r}
oak_polygons|>as_tibble()|>
 group_by(holc_grade)|>
 summarise(mean_NDVI=mean(NDVI))
```
