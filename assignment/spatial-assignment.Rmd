...
```
We are examining historical redlining maps developed during the Great Depression for Oakland, San Francisco, and San Jose. 
```{r}
...
ndvi <- 
  ...
  aggregate_time("P3M")
#add geometries back on to table 
...
tm_shape(ndvi2)+ tm_raster()+ tm_shape(sf) + tm_polygons("holc_grade", alpha=0.5)
plot(ndvi2)
plot(ndvi)
```
